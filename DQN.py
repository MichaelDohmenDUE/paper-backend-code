...
def epsilon_greedy(q_values: torch.tensor, epsilon: float) -> torch.tensor:
    ...
    return random.choice(actions) if epsilon < random.random() else greedy_action
...
if __name__ == '__main__':
    ...
    behavior_policy = MultiLayerPerceptron(state_size=env.observation_space.shape[0], hidden_size=64,
                                           action_size=env.action_space.n, activation_func=nn.ReLU())
    target_policy = MultiLayerPerceptron(state_size=env.observation_space.shape[0], hidden_size=64,
                                         action_size=env.action_space.n, activation_func=nn.ReLU())
